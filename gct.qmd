...
us_all_imports <- gct %>%
  ...
  mutate(
    crisis = if_else(year > 2008, 1, 0)
  ) %>%
  drop_na(trade_usd, year, crisis)

model <- lm(trade_usd ~ year + crisis, data = us_all_imports)
summary(model)
```
...
# Plot trade value over time with crisis periods colored
ggplot(us_all_imports, aes(x = year, y = trade_usd)) +
  geom_point(alpha = 0.3, color = "sienna4") +
  geom_smooth(method = "lm", formula = y ~ x + crisis, color = "saddlebrown", se = TRUE) +
  scale_y_continuous(labels = scales::dollar_format(scale = 1e-9, suffix = "B")) +
  labs(
    title = "U.S. Import Trade Value Over Time",
    subtitle = "Linear model with pre/post 2008 financial crisis effect",
    x = "Year",
    y = "Trade Value (Billions USD)",
    caption = "Data: Global Commodity Trade Statistics"
  ) +
